...
class LunarLander(gym.Env, EzPickle):
    ...
    def step(self, action):
        ...
        result_reward = 0
        self.steps += 1
        if self.game_over or abs(state[0]) >= 1.0 or self.steps > self._max_episode_steps:
            done = True
            result_reward = -100
            reward = -100
        ...
        return np.array(state, dtype=np.float32), np.array([shaping_reward, result_reward, -m_power, -s_power, time_penalty]), done, {}
...
register(id='MO_LunarLander5d-v0', entry_point='environments.MO_lunar_lander5d:LunarLanderContinuous')
